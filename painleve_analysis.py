...
class PainleveAnalysis:
    ...
    def plot_resonance_structure(self, sigma_values: np.ndarray,
                              figsize: Tuple[float, float] = (12, 6)) -> plt.Figure:
        """
        Plot the resonance structure for different mass parameters.
        ...
        Returns:
            The figure object
        """
        # Analyze resonances for each sigma value
        results = []
        for sigma in sigma_values:
            # For homothetic orbits
            result = self.analyze_three_body_homothetic(sigma)
            results.append({
                "sigma": sigma,
                "resonances": result["resonances"],
                "compatibility": result["compatibility_satisfied"],
                "branch_type": result["branch_point_type"]
            })

        # Create the figure
        fig, ax = plt.subplots(figsize=figsize)

        # Plot resonances as points
        for i, result in enumerate(results):
            ...
            # Determine color based on branch point type
            if "square root" in branch_type:
                color = 'green'
            elif "none" in branch_type:
                color = 'blue'
            else:
                color = 'red'
            ...
            for res in resonances:
                if res >= 0:  # Skip the -1 resonance (arbitrary singularity location)
                    ax.scatter(sigma, res, c=color, s=100, alpha=0.7)
                    # Add annotation for key points
                    if i == 0 or i == len(results) - 1 or abs(sigma - 1/3) < 1e-5 or abs(sigma - 2**3/3**3) < 1e-5 or abs(sigma - 2/3**2) < 1e-5:
                        ax.text(sigma, res, f"j={res}", ha='right', va='bottom')
        ...
        for val in exceptional_values:
            ...
            ax.text(val, 5.5, f"σ = {val:.6f}",
                   rotation=90, va='top', ha='center')

        # Add a legend for branch point types
        ax.scatter([], [], c='red', label='Transcendental branch points', s=100)
        ax.scatter([], [], c='green', label='Square root branch points (Z_2)', s=100)
        ax.scatter([], [], c='blue', label='No branch points (meromorphic)', s=100)

        # Set plot properties
        ax.set_xlabel('Mass parameter σ')
        ax.set_ylabel('Resonance index j')
        ax.set_title('Resonance Structure in Painlevé Analysis for Different Mass Ratios')
